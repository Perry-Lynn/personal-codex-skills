...
脚本严格按 CSV 行顺序处理，不排序、不跨 cohort 或其他分组寻找上一行。缺失、非法、`NaN` 或无穷数值输出为 `null`，并在该行的 `diagnostics` 中说明；缺失值会断开相邻行变化计算。末尾单个 `%` 按百分比转为比例（`50%` → `0.5`），普通数字 `50` 保持为 `50`。显式数值列、比率列、章节列和比率表达式必须存在且不重名；比率名称不得覆盖 `chapter`、`diagnostics`、结果保留字段、显式数值列或任意 `*_change` 字段。均值先缩放后稳定求和，有限输入不会因求和溢出生成非标准 JSON；零分母输出 `null` 并诊断。输入或契约错误返回 `2`，有效分析完成返回 `0`。
