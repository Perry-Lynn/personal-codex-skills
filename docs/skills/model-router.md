...
`model-router` 是一个可配置的 Codex 开发协作 skill，用两个独立对话分工完成复杂开发任务。默认配置是：

- Astra（`gpt-6-astra`，`low`）负责只读分析、方案和最终审查。
- Luna（`gpt-5.6-luna`，`high`）负责读取方案、修改代码、调试和测试。

用户可以在每次请求中指定谁来规划、谁来实施，也可以分别覆盖两个角色的模型和推理强度。例如：`Astra low 规划、Luna high 实施`、`gpt-6-astra/medium 规划，gpt-5.6-luna/high 实施`，或交换两者职责。只指定一侧时，另一侧沿用默认配置。
...
```text
使用 $model-router。
Planner 使用 gpt-6-astra / low，Executor 使用 gpt-5.6-luna / high。
```
...
Skill 不能改变当前对话模型，也不修改全局 `config.toml`。`thinking` 是 Codex 任务工具使用的推理强度字段，对应用户所说的 `reasoning_effort`。工具未提供后端模型身份元数据时，应说明只能确认调用参数被接受；不支持用户指定的模型/强度组合时，应报告阻塞而不是静默替换。
