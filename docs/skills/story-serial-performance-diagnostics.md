...
脚本按 CSV 原始行顺序计算；缺失、非法、`NaN` 或无穷数值输出为 `null` 并记录行级 `diagnostics`，缺失会断开变化基准。末尾单个 `%` 转为比例（`50%` → `0.5`），普通 `50` 不转换。CSV 输入表头和显式列不得使用 `chapter`、`diagnostics`、结果保留字段或任意 `*_change` 派生字段；比率引用和比率名称必须存在且不重名，比率名称也不得覆盖这些字段。均值采用稳定求和，有限输入不会因累加溢出；零分母为 `null` 并诊断。契约或输入错误返回 `2`，有效分析完成返回 `0`。
